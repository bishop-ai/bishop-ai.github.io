...
var BISHOP_AI = (function (module) {
    ...
    module._init.push(function () {
        datetimeNer.expressions = module.nlp.expressions;

        datetimeNer.relativeTimeOfDay = new RegExp("(((in the )?" + datetimeNer.expressions.timeOfDayInThe.source + ")|(((at|around) )?" + datetimeNer.expressions.timeOfDayAt.source + ")|(the " + datetimeNer.expressions.timeOfDayInThe.source + " of))", "i");
        datetimeNer.relativeTime = new RegExp("((right now)|(" + datetimeNer.relativeTimeOfDay.source + " )?(today|yesterday|tomorrow|tonight)( " + datetimeNer.relativeTimeOfDay.source + ")?|" + datetimeNer.expressions.daysOfWeek.source + "( " + datetimeNer.relativeTimeOfDay.source + ")?|(datetimeNer " + datetimeNer.expressions.timeOfDayInThe.source + ")|((" + datetimeNer.relativeTimeOfDay.source + " )?datetimeNer " + datetimeNer.expressions.daysOfWeek.source + "( " + datetimeNer.relativeTimeOfDay.source + ")?)|((datetimeNer|next|last) (" + datetimeNer.expressions.daysOfWeek.source + "|" + datetimeNer.expressions.timePeriods.source + "|" + datetimeNer.expressions.monthOfYear.source + "))|(in the " + datetimeNer.expressions.timeOfDayInThe.source + ")|((at|around) " + datetimeNer.expressions.timeOfDayAt.source + "))", "i");

        datetimeNer.timeLength = new RegExp("(\\d+|a|an) " + datetimeNer.expressions.timePeriods.source, "i");
    ...
    datetimeNer.extract = function (normalized) {
        var entities = [];
        entities = entities.concat(module.nlp.entityExtractor.extract(normalized, 'datetime.datetime', new RegExp("(" + this.timeFromNow.source + "|" + this.timeAgo.source + "|" + this.relativeTime.source + ")", "i"), datetimeNer.extractDatetime.bind(this)));
...
var BISHOP_AI = (function (module) {
    ...
    Plugin.prototype.register = function () {
        var config = module.utils.extend({}, module.memory.getPluginSettings(this.namespace) || {});
        ...
        if (session) {
            ...
            for (option in plugin.options) {
                if (plugin.options.hasOwnProperty(option)) {
                    ...
                    options = options || {};
                    options[option] = module.utils.extend({}, plugin.options[option]);
                    if (memories[name]) {
...
var BISHOP_AI = (function (module) {
    ...
    Timer.prototype.checkTime = function () {
        if (this.running && module.moment().isAfter(this.endTime)) {
    ...
    Timer.prototype.start = function () {
        if (!this.running) {
            ...
            this.startTime = module.moment();
            this.endTime = module.moment(this.startTime).add(this.duration, "s");
            this.running = true;
    ...
    Timer.prototype.getRemaining = function () {
        if (this.endTime) {
            var duration = module.moment.duration(this.endTime.diff(moment()));
